...
public class ProductSupervisorFrame extends JFrame {
    ...
    private DefaultTableModel productsDefTableModel;
    private JTable productsDefTable;
    private DefaultTableModel reqsTableModel;
    ...
    private JPanel createLogPanel() {
        ...
        logArea.setBackground(new Color(45, 45, 50));
    ...
    private JPanel createInventoryPanel() {
        ...
        JButton btnSearchItems = new JButton("🔍 Search");
        ...
        btnSaveInventory.addActionListener(e -> {
            // Inventory.saveToFile(); // تم تعطيل هذا السطر لأنه يسبب فقدان بيانات الفئة
            JOptionPane.showMessageDialog(this, "Inventory is saved automatically on each change.", "Info", JOptionPane.INFORMATION_MESSAGE);
        });
    ...
    private JPanel createProductDefsPanel() {
        ...
        productsPanel.add(new JScrollPane(productsDefTable), BorderLayout.CENTER);

        JPanel productButtons = new JPanel(new FlowLayout(FlowLayout.CENTER));
        setupPanel(productButtons);
        ...
        JPanel reqsPanel = new JPanel(new BorderLayout(5,5));
        setupPanel(reqsPanel);
        reqsPanel.setBorder(createTitledBorder("Required Materials for Selected Product"));
        ...
        reqsPanel.add(new JScrollPane(reqsTable), BorderLayout.CENTER);

        JPanel reqsButtons = new JPanel(new FlowLayout(FlowLayout.CENTER));
        setupPanel(reqsButtons);
    ...
    private JPanel createAnalyticsPanel() {
        ...
        setupPanel(allProductsPanel);
        allProductsPanel.setBorder(createTitledBorder("All Manufactured Products"));
        JButton runAllProducts = new JButton("Show All Products");
        JTextArea allProductsArea = new JTextArea();
        allProductsPanel.add(runAllProducts, BorderLayout.NORTH);
        allProductsPanel.add(new JScrollPane(styleTextArea(allProductsArea)), BorderLayout.CENTER);
        runAllProducts.addActionListener(e->{
            Inventory.loadFromFile();
            String result = Inventory.getProdInv().keySet().stream().collect(Collectors.joining("\n"));
            allProductsArea.setText(result.isEmpty() ? "No products manufactured yet." : result);
        });
    ...
    private void handleUpdateTaskStatus() {
        ...
        if (taskToUpdate != null) {
            ...
            if (newStatus != null) {
                ...
                JOptionPane.showMessageDialog(this, "Task" + taskId + " status updated!");
                refreshTasksTable();
            }
        }
    }
    ...
    private void handleAddRequirement() {
        ...
        try {
            ...
            if (quantity <= 0) {
                Task.WriteErrMsgs("The quantity must be negative");
    ...
    private JTextArea styleTextArea(JTextArea textArea) {
        textArea.setBackground(new Color(50, 50, 55));
    ...
    private JPanel createReportPanel(String title, String label, boolean useComboBox, java.util.function.BiConsumer<String, JTextArea> action) {
        ...
        panel.add(new JScrollPane(styleTextArea(resultArea)), BorderLayout.CENTER);
    ...
    private JButton createActionButton(String text) {
        ...
        button.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) { button.setBackground(new Color(90, 90, 95)); }
            public void mouseExited(java.awt.event.MouseEvent evt) { button.setBackground(new Color(70, 70, 75)); }
        });
        return button;
    }
